...
# Load data
df = pd.read_csv(os.path.join(clean_dir, 'preprocessed_dataset.csv'), parse_dates=['Date Outbreak Began', 'Date Declared Over'])
...
# Create binary flags on the raw df  one-hot encoding
...
print("Sample agent dummies:",[c for c in df_encoded.columns if c.startswith('Causative Agent-1_')][:5],[c for c in df_encoded.columns if c.startswith('Causative Agent-2_')][:5])

# Define feature matrix X and target y
exclude = ['_id','Institution Name','Institution Address','Date Outbreak Began','Date Declared Over','Severity','Severity_Label']
...
# Save processed dataset to Cleaned Datasets
processed_path = os.path.join(clean_dir, 'processed_outbreak_dataset.csv')
processed_df.to_csv(processed_path, index=False)
print(f"Saved 'processed_outbreak_dataset.csv'")

# Save scaler and label encoder to Output Files
scaler_path = os.path.join(out_dir, 'scaler.pkl')
encoder_path = os.path.join(out_dir, 'severity_label_encoder.pkl')
joblib.dump(scaler, scaler_path)
joblib.dump(le, encoder_path)
print(f"Saved 'scaler.pkl' and 'severity_label_encoder.pkl'")
